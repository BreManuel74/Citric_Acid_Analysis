...
_MODE_LABELS = {
    'ramp': {
        ...
        'tukey_empty_msg':   'No significant CA% or interaction effects found - post-hoc tests not needed.',
        'tukey_desc':        'Paired t-tests with Bonferroni correction for within-subjects CA% comparisons.\nCA% is a within-subjects factor (same animals experience all CA% levels).',
        'tukey_main_hdr':    'CA% Main Effect - Pairwise Comparisons (collapsed across Sex):',
        'tukey_interact_hdr':'Sex × CA% Interaction - Simple Effects (CA% comparisons within each Sex):',
    ...
    'nonramp': {
        ...
        'tukey_empty_msg':   'No significant Week or interaction effects found - post-hoc tests not needed.',
        'tukey_desc':        'Paired t-tests with Bonferroni correction for within-subjects Week comparisons.\nWeek is a within-subjects factor (same animals measured each week).',
        'tukey_main_hdr':    'Week Main Effect - Pairwise Comparisons (collapsed across Sex):',
        'tukey_interact_hdr':'Sex × Week Interaction - Simple Effects (Week comparisons within each Sex):',
	...
	for measure_name, column in measures.items():
		...
		try:
			if within_significant:
				...
				tukey_results[measure_name][pk] = {'comparisons': comparisons}
...
def display_tukey_hsd_results(tukey_results: dict) -> str:
	...
	for measure, results in tukey_results.items():
		...
		if results.get(pk):
			lines.append(_MLB['tukey_main_hdr'])
			lines.append("-"*60)
		...
		if results.get('simple_effects'):
			lines.append(f"{_MLB['tukey_interact_hdr']}")
			lines.append("-"*60)
			lines.append("")
...
def main() -> None:
	...
	try:
		...
		print(f"\nPerforming Tukey HSD post-hoc tests for significant {fl} effects...")
