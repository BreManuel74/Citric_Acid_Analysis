...
def compute_sensor_KDE(df: pd.DataFrame, sensor_cols: List[str], cache_file: Path = None) -> pd.Series:
        ...
        try:
            ...
            for ca_percent in sorted(df_measure['CA_Percent'].unique()):
                ...
                group_stats.append({
                    ...
                    'n': n,
                    'mean': mean,
                    'median': ca_data.median(),
                    'std': std,
                    'sem': sem,
                    'ci_lower': ci_lower,
                    'ci_upper': ci_upper,
                    'q25': ca_data.quantile(0.25),
                    'q75': ca_data.quantile(0.75),
                    'min': ca_data.min(),
                    'max': ca_data.max()
                })
...
def perform_mixed_anova_posthoc(data: pd.DataFrame, dv: str, within: str, between: str, subject: str, 
    ...
    1. Within-subjects pairwise comparisons (across CA% levels)
    2. Between-subjects pairwise comparisons (between sexes)
    3. Simple effects analysis (CA% effect at each sex level)
    
    Parameters:
        data: DataFrame with long-format data
        dv: Dependent variable column name
        within: Within-subjects factor column (e.g., 'CA%')
    ...
    # 1. Within-subjects pairwise comparisons (CA% pairs, collapsed across Sex)
    try:
    ...
    # 2. Between-subjects pairwise comparisons (Sex pairs, collapsed across CA%)
    try:
        # Average across CA% levels for each subject first
        between_data = data.groupby([subject, between])[dv].mean().reset_index()
        
        sex_levels = between_data[between].unique()
        if len(sex_levels) == 2:
            # For 2 groups, do independent t-test
            group1 = between_data[between_data[between] == sex_levels[0]][dv]
            group2 = between_data[between_data[between] == sex_levels[1]][dv]
    ...
    # 3. Simple effects: CA% effect at each sex level
    try:
        sex_levels = data[between].unique()
        simple_effects = []
        
        for sex in sex_levels:
            sex_data = data[data[between] == sex]
            
            # Perform repeated measures ANOVA for this sex
            ca_levels = sex_data[within].unique()
            if len(ca_levels) >= 2:
...
def display_posthoc_results(posthoc_results: Dict, measure_name: str) -> str:
    ...
    # 1. Within-subjects pairwise (CA% comparisons)
    lines.append("1. WITHIN-SUBJECTS PAIRWISE COMPARISONS (CA% levels)")
    lines.append("-" * 80)
    ...
    if isinstance(within_pw, str):
        ...
        lines.append("   Pairwise comparisons across CA% concentrations (repeated measures):")
        lines.append("")
    ...
    # 2. Between-subjects pairwise (Sex comparisons)
    lines.append("2. BETWEEN-SUBJECTS PAIRWISE COMPARISONS (Sex)")
    lines.append("-" * 80)
    ...
    if isinstance(between_pw, str):
        ...
        lines.append("   Comparison between sexes (collapsed across CA% levels):")
        lines.append("")
    ...
    # 3. Simple effects (CA% at each sex)
    ...
    if isinstance(simple_eff, str):
        ...
        lines.append("   CA% effect separately for each sex:")
        lines.append("")
        for idx, row in simple_eff.iterrows():
            ...
            if row['significant']:
                lines.append(f"      *** SIGNIFICANT CA% effect for {row['Sex']} ***")
            else:
...
def plot_first_5min_by_week(
    ...
    # Define colors based on CA% - gradient from blue (low) to red (high)
    # Map CA% to colors
    ca_colors = []
    for ca in ca_percents:
        if ca == 0:
            ca_colors.append('dodgerblue')
        elif ca <= 1.0:
            ca_colors.append('skyblue')
        elif ca <= 2.0:
            ca_colors.append('gold')
        elif ca <= 3.0:
            ca_colors.append('orange')
        else:
            ca_colors.append('orangered')
    ...
    ax.set_title('Percentage of Licks in First 5 Minutes Across Weeks\n(CA% Ramp Design)',
                fontsize=14, weight='bold')
...
def plot_first_5min_by_week_with_lines(
    ...
    # Plot bars
    bars = ax.bar(x_positions, avg_pcts, bar_width,
                 color=ca_colors, alpha=0.6, edgecolor='black', linewidth=1.5, zorder=5)
    ...
    for idx, (animal_id, trajectory) in enumerate(animal_trajectories.items()):
        ...
        if len(trajectory) > 1:  # Only draw lines if animal appears in multiple weeks
            ...
            ax.plot(weeks, pcts, color=colors[idx], alpha=0.6, linewidth=1.5, zorder=3)
    ...
    ax.set_xlabel('Week', fontsize=12, weight='bold')
    ax.set_ylabel('% of Licks in First 5 Minutes', fontsize=12, weight='bold')
    ax.set_title('Individual Mouse Trajectories of Lick Percentage Across Weeks\n(CA% Ramp Design)',
                fontsize=14, weight='bold')
    ax.set_xticks(x_positions)
    ax.set_xticklabels(week_labels)
    ax.set_ylim(bottom=0)
...
def plot_time_to_50pct_by_week(
    ...
    In the ramp design, all mice experience the same CA% in each week, with concentration
    increasing across weeks. This creates a single plot with one bar per week showing
    average time with individual mouse data points overlaid and SEM error bars.
    ...
    ax.set_title('Time to Reach 50% of Total Licks Across Weeks\n(CA% Ramp Design)',
                fontsize=14, weight='bold')
    ax.set_xticks(x_positions)
    ax.set_xticklabels(week_labels)
    ax.set_ylim(bottom=0)
    ...
    for i, (bar, ca_pct) in enumerate(zip(bars, ca_percents)):
        height = bar.get_height()
        ax.text(bar.get_x() + bar.get_width()/2., height + sem_times[i] + 0.5,
               f'{ca_pct}% CA',
               ha='center', va='bottom', fontsize=9, weight='bold')
...
def plot_time_to_50pct_by_week_with_lines(
    ...
    """Create bar plot with individual mouse trajectories for time to 50% of total licks.
    ...
    for idx, (animal_id, trajectory) in enumerate(animal_trajectories.items()):
        ...
        if len(trajectory) > 1:  # Only draw lines if animal appears in multiple weeks
            ...
            # Draw line connecting this animal's data points
            ax.plot(weeks, times, color=colors[idx], alpha=0.6, linewidth=1.5, zorder=3)
    ...
    ax.set_xlabel('Week', fontsize=12, weight='bold')
    ax.set_ylabel('Time to 50% of Total Licks (minutes)', fontsize=12, weight='bold')
    ax.set_title('Individual Mouse Trajectories of Time to 50% Licks Across Weeks\n(CA% Ramp Design)',
                fontsize=14, weight='bold')
    ax.set_xticks(x_positions)
    ax.set_xticklabels(week_labels)
    ax.set_ylim(bottom=0)
...
def process_single_week(
    ...
    sensor_kdes = compute_sensor_KDE(df, sensor_cols, cache_file=cache_file)
...
def main():
    ...
    # Plot time to 50% of total licks by week with individual mouse trajectories
    ...
    save_time_50pct_lines = input("\nSave time to 50% of total licks plot with mouse trajectories as SVG? (y/n): ").strip().lower()
